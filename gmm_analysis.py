...
def gmm_cluster(demo_df):
    ...
    gmm_model_fp = os.path.abspath('') + '/Models/gmm.joblib'
    try:
        model = load(gmm_model_fp)
        print(f'Loaded Gaussian Mixture Model from {gmm_model_fp}')
        num_clusters = len(model.weights_)
    except:
        # Group users by demographic using Gaussian Mixture
        ...
        for n in num_components: 
            ...
            model = GaussianMixture(n_components=n, init_params='kmeans', random_state=10).fit(X) 
            model.fit(X)     
        
            bic_scores.append(model.bic(X))

        # visualize BIC method - Shows that the best number of clusters is: 5
        plt.plot(num_components, bic_scores, 'bx-') 
        plt.xlabel('# Components') 
        plt.ylabel('BIC Score') 
        plt.title('The Bayesian Information Criterion (BIC) by number of components') 
        plt.show()    
        
        # get cluster labels
        num_clusters = 5
        print('Fitting model...')
        model = GaussianMixture(n_components=num_clusters, init_params='kmeans', random_state=10).fit(X) 

        try:
            dump(model, gmm_model_fp)
            print(f'Saved model to {gmm_model_fp}')
        except:
            print(f'Could not save model to {gmm_model_fp}')

    # label user by cluster
    labels = model.predict(X)
    demo_df['gmm_cluster'] = labels
    return num_clusters, demo_df, labels


def count_offer_interactions(profile_offer_df):
    # get data for clusters and offer interactions only
    cluster_offer_col_list = ['gmm_cluster']
    # add column for each offer
    for i in range(1,11):
    ...
    for cluster_num in range(1, num_clusters+1):
        ...
        cluster = cluster_offer_df[cluster_offer_df['gmm_cluster']==cluster_num]
...
if __name__ == "__main__":
    ...
    offer_interaction_counts_fp = os.path.abspath('') + '/Data/offer_interaction_counts_gmm.pkl'
    ...
    try:
        ...
        # use Gaussian Mixture to cluster customers
        num_clusters, clustered_demo_df, labels = gmm_cluster(demo_df)
        profile_offer_df['gmm_cluster'] = labels+1     

    # create df if it does not exist
    except:
        print(f'Could not load offer_interaction_counts_gmm and/or profile_offer_df pickle files from /Data')
        # get demographic data as separate dataframe for clustering
        demo_df = create_demo_df(profile_offer_df)
    ...
    cluster_demo_data = profile_offer_df[['gender', 'age', 'member_time_days', 'income', 'gmm_cluster']]
    # compare each demographic metric by cluster
    demo_subplots = make_subplots(
        rows=1, cols=5,
        subplot_titles = ('# Ppl', 'Gender %', 'Age', 'Member', 'Income')   
    )
    subplot_col=1
    # add the number in each cluster
    len_clusters = []
    cluster_nums = []
    for cluster_num in cluster_demo_data['gmm_cluster'].unique():
        cluster = cluster_demo_data[cluster_demo_data['gmm_cluster']==cluster_num]
    ...
    subplot_col += 1
    for col_name in cluster_demo_data.drop('gmm_cluster', axis=1).columns:
        if col_name == 'gender':
            f_pct = []
            m_pct = []
            o_pct = []
    ...
    irr_fig.update_layout(title='Incremental Response Rate by Cluster for each Offer #', barmode='group')
    irr_fig.show()
